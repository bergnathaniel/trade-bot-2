"""
Phase 6: Santa Claus rally, weekend effect, January small caps, presidential cycle (PREREG_PHASE6.md).
====================================================================================================

    python3 research/run_phase6.py

Same harness and gates as Phases 1-5. Prints the SHA-256 of PREREG_PHASE6.md, writes
research/results/phase6.json and refreshes the P6 rows of REGISTRY.csv (other rows untouched).
...
import h_phase6  # noqa: E402
import run_phase1 as p1  # noqa: E402
import sources  # noqa: E402
import stats  # noqa: E402
from h_common import slice_from, spy_excess  # noqa: E402

PREREG = os.path.join(HERE, "PREREG_PHASE6.md")
OUT = os.path.join(HERE, "results", "phase6.json")
N_TRIALS = 112  # 108 through Phase 5 + these 4


def update_registry(results):
    with open(p1.REGISTRY, newline="") as f:
        rows = list(csv.DictReader(f))
    ids = {f"P6-{x['meta']['id']}" for x in results}
    keep = [r for r in rows if r["id"] not in ids]
    today = time.strftime("%Y-%m-%d")
    for x in results:
        m, o = x["meta"], x["metrics_oos"]
        keep.append({
            "id": f"P6-{m['id']}", "date": today, "phase": "phase6", "cluster": m["cluster"],
            ...
            "n_configs": 1 + x["n_neighbours"], "status": x["status"], "file": "research/run_phase6.py",
    ...
    with open(p1.REGISTRY, "w", newline="") as f:
        ...
        w.writeheader()
        w.writerows(keep)
    print(f"updated REGISTRY.csv ({len(results)} Phase-6 rows)")
...
def main():
    ...
    print(p1.BAR)
    print("PHASE 6 - Santa Claus rally, weekend effect, January small caps, presidential cycle (pre-registered)")
    print(f"PREREG_PHASE6.md sha256 = {digest}")
    print(p1.BAR)
    ...
    bench = {"SPY": spy_excess(), "MKT": mktrf}

    records = []
    for label, fn in (("H38", h_phase6.h38), ("H39", h_phase6.h39), ("H40", h_phase6.h40), ("H41", h_phase6.h41)):
        t = time.time()
        got = fn()
        records += got
    ...
    print(f"deflated-Sharpe trial count N = {N_TRIALS}; SR dispersion: Phase-6 {stats.stdev(oos_sr):.3f}, "
