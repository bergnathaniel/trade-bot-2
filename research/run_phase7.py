"""
Phase 7: IBS mean reversion in SPY and QQQ (PREREG_PHASE7.md).
==============================================================

    python3 research/run_phase7.py

Same harness and gates as Phases 1-6. Prints the SHA-256 of PREREG_PHASE7.md, writes
research/results/phase7.json and refreshes the P7 rows of REGISTRY.csv (other rows untouched).
...
PREREG = os.path.join(HERE, "PREREG_PHASE7.md")
OUT = os.path.join(HERE, "results", "phase7.json")
N_TRIALS = 114  # 112 through Phase 6 + these 2


def update_registry(results):
    with open(p1.REGISTRY, newline="") as f:
        rows = list(csv.DictReader(f))
    ids = {f"P7-{x['meta']['id']}" for x in results}
    keep = [r for r in rows if r["id"] not in ids]
    today = time.strftime("%Y-%m-%d")
    for x in results:
        m, o = x["meta"], x["metrics_oos"]
        keep.append({
            "id": f"P7-{m['id']}", "date": today, "phase": "phase7", "cluster": m["cluster"],
            ...
            "n_configs": 1 + x["n_neighbours"], "status": x["status"], "file": "research/run_phase7.py",
    ...
    with open(p1.REGISTRY, "w", newline="") as f:
        ...
        w.writeheader()
        w.writerows(keep)
    print(f"updated REGISTRY.csv ({len(results)} Phase-7 rows)")
...
def main():
    ...
    print(p1.BAR)
    print("PHASE 7 - IBS mean reversion in SPY and QQQ (pre-registered)")
    print(f"PREREG_PHASE7.md sha256 = {digest}")
    print(p1.BAR)
    ...
    bench = {"SPY": spy_excess(), "QQQ": h_phase7.excess_map("QQQ"), "MKT": mktrf}

    records = []
    for label, fn in (("H42", h_phase7.h42), ("H43", h_phase7.h43)):
        t = time.time()
        got = fn()
        records += got
    ...
    print(f"deflated-Sharpe trial count N = {N_TRIALS}; SR dispersion: Phase-7 {stats.stdev(oos_sr):.3f}, "
