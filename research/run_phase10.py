"""
Phase 10: the overnight drift in SPY and QQQ (PREREG_PHASE10.md).
===================================================================

    python3 research/run_phase10.py

Same harness and gates as Phases 1-9. Prints the SHA-256 of PREREG_PHASE10.md, checks the 5-year
close-to-open vs. open-to-close split quoted in the pre-registration against this harness's own
numbers, writes research/results/phase10.json and refreshes the P10 rows of REGISTRY.csv (other
rows untouched).
...
import h_phase7  # noqa: E402
import h_phase10  # noqa: E402
import run_phase1 as p1  # noqa: E402
import sources  # noqa: E402
import stats  # noqa: E402
from h_common import slice_from, spy_excess  # noqa: E402

PREREG = os.path.join(HERE, "PREREG_PHASE10.md")
OUT = os.path.join(HERE, "results", "phase10.json")
N_TRIALS = 120  # 118 through Phase 9 + these 2
...
def update_registry(results):
    ...
    ids = {f"P10-{x['meta']['id']}" for x in results}
    keep = [r for r in rows if r["id"] not in ids]
    today = time.strftime("%Y-%m-%d")
    for x in results:
        m, o = x["meta"], x["metrics_oos"]
        keep.append({
            "id": f"P10-{m['id']}", "date": today, "phase": "phase10", "cluster": m["cluster"],
            ...
            "n_configs": 1 + x["n_neighbours"], "status": x["status"], "file": "research/run_phase10.py",
    ...
    with open(p1.REGISTRY, "w", newline="") as f:
        ...
        w.writeheader()
        w.writerows(keep)
    print(f"updated REGISTRY.csv ({len(results)} Phase-10 rows)")
...
def main():
    ...
    print(p1.BAR)
    print("PHASE 10 - the overnight drift in SPY and QQQ (pre-registered)")
    print(f"PREREG_PHASE10.md sha256 = {digest}")
    print(p1.BAR)
    ...
    spy_ex = spy_excess()
    bench = {"SPY": spy_ex, "QQQ": h_phase7.excess_map("QQQ"), "MKT": mktrf}

    print("checking the 5-year close-to-open vs open-to-close split quoted in the pre-registration...")
    for sym in ("SPY", "QQQ"):
        print(f"  {sym}: {h_phase10.check_five_year_split(sym)}")

    records = []
    for label, fn in (("H48", h_phase10.h48), ("H49", h_phase10.h49)):
        t = time.time()
        got = fn(spy_ex)
        records += got
    ...
    print(f"deflated-Sharpe trial count N = {N_TRIALS}; SR dispersion: Phase-10 {stats.stdev(oos_sr):.3f}, "
