"""
Phase 8: country momentum rotation and G10 currency carry (PREREG_PHASE8.md).
============================================================================

    python3 research/run_phase8.py

Same harness and gates as Phases 1-7. Prints the SHA-256 of PREREG_PHASE8.md, writes
research/results/phase8.json and refreshes the P8 rows of REGISTRY.csv (other rows untouched).
...
import h_phase8  # noqa: E402
import run_phase1 as p1  # noqa: E402
import sources  # noqa: E402
import stats  # noqa: E402
from h_common import slice_from, spy_excess  # noqa: E402
from h_phase5 import ew5_excess  # noqa: E402

PREREG = os.path.join(HERE, "PREREG_PHASE8.md")
OUT = os.path.join(HERE, "results", "phase8.json")
N_TRIALS = 116  # 114 through Phase 7 + these 2


def update_registry(results):
    with open(p1.REGISTRY, newline="") as f:
        rows = list(csv.DictReader(f))
    ids = {f"P8-{x['meta']['id']}" for x in results}
    keep = [r for r in rows if r["id"] not in ids]
    today = time.strftime("%Y-%m-%d")
    for x in results:
        m, o = x["meta"], x["metrics_oos"]
        keep.append({
            "id": f"P8-{m['id']}", "date": today, "phase": "phase8", "cluster": m["cluster"],
            ...
            "n_configs": 1 + x["n_neighbours"], "status": x["status"], "file": "research/run_phase8.py",
    ...
    with open(p1.REGISTRY, "w", newline="") as f:
        ...
        w.writeheader()
        w.writerows(keep)
    print(f"updated REGISTRY.csv ({len(results)} Phase-8 rows)")
...
def main():
    ...
    print(p1.BAR)
    print("PHASE 8 - country momentum rotation and G10 currency carry (pre-registered)")
    print(f"PREREG_PHASE8.md sha256 = {digest}")
    print(p1.BAR)
    ...
    spy = spy_excess()
    bench = {"SPY": spy, "MKT": mktrf, "EW17": ew5_excess(h_phase8.COUNTRIES)}

    records = []
    for label, fn in (("H44", h_phase8.h44), ("H45", lambda: h_phase8.h45(spy))):
        t = time.time()
        got = fn()
        records += got
    ...
    print(f"deflated-Sharpe trial count N = {N_TRIALS}; SR dispersion: Phase-8 {stats.stdev(oos_sr):.3f}, "
