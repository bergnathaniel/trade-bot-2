"""
Phase 4: dual momentum, Halloween, FOMC-cycle weeks (PREREG_PHASE4.md), end to end.
==================================================================================

    python3 research/run_phase4.py

Same harness and gates as Phases 1-2. Prints the SHA-256 of PREREG_PHASE4.md, writes
research/results/phase4.json and refreshes the P4 rows of REGISTRY.csv (other rows untouched).
...
import h_phase4  # noqa: E402
import run_phase1 as p1  # noqa: E402
import sources  # noqa: E402
import stats  # noqa: E402
from h_common import slice_from, spy_excess  # noqa: E402

PREREG = os.path.join(HERE, "PREREG_PHASE4.md")
OUT = os.path.join(HERE, "results", "phase4.json")
N_TRIALS = 104  # 76 legacy + 17 Phase-1 + 4 Phase-2 + 3 intraday + TERM + these 3


def update_registry(results):
    with open(p1.REGISTRY, newline="") as f:
        rows = list(csv.DictReader(f))
    ids = {f"P4-{x['meta']['id']}" for x in results}
    keep = [r for r in rows if r["id"] not in ids]
    today = time.strftime("%Y-%m-%d")
    for x in results:
        m, o = x["meta"], x["metrics_oos"]
        keep.append({
            "id": f"P4-{m['id']}", "date": today, "phase": "phase4", "cluster": m["cluster"],
            ...
            "n_configs": 1 + x["n_neighbours"], "status": x["status"], "file": "research/run_phase4.py",
    ...
    with open(p1.REGISTRY, "w", newline="") as f:
        ...
        w.writeheader()
        w.writerows(keep)
    print(f"updated REGISTRY.csv ({len(results)} Phase-4 rows)")
...
def main():
    ...
    print(p1.BAR)
    print("PHASE 4 - dual momentum, Halloween, FOMC-cycle weeks (pre-registered)")
    print(f"PREREG_PHASE4.md sha256 = {digest}")
    print(p1.BAR)
    ...
    records = []
    for label, fn in (("H31", h_phase4.h31), ("H32", h_phase4.h32), ("H33", h_phase4.h33)):
        t = time.time()
        got = fn()
        records += got
    ...
    print(f"deflated-Sharpe trial count N = {N_TRIALS}; SR dispersion: Phase-4 {stats.stdev(oos_sr):.3f}, "
